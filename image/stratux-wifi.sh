...
function hostapd-upgrade {
	...
	wLog "Moving existing valuse from $DAEMON_CONF to $DAEMON_USER_PREF if found"
...
function ap-start {
	...
	wLog "Making new hostpad.conf file at $DAEMON_TMP with $DAEMON_CONF and $DAEMON_USER_PREF"
	cat ${DAEMON_USER_PREF} <(echo) ${DAEMON_CONF} > ${DAEMON_TMP}

	wLog "Starting hostapd. $DAEMON_SBIN -B $DAEMON_TMP"
