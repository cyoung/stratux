...
func processNMEALine(l string) bool {
	...
	if x[0] == "$GNVTG" { // Ground track information.
	...
	} else if x[0] == "$GPGGA" { // GPS fix.
		log.Printf("GPGGA Message\n")
		log.Printf("%s", x)

		// $GPGGA 194718.000 3915.0856 N 09426.2650 W 2 9 0.87 310.7 M -30.6 M 0000 0000*69
		...
		// Timestamp.
		if len(x[1]) < 9 {
			log.Printf("Timestamp less than 9\n")
			return false
		}
		...
		// Latitude.
		// [$GPGGA 195448.000 3915.0825 N 09426.2648 W 2 7 1.39 307.5 M -30.6 M 0000 0000*6D]
		if len(x[2]) < 9 {
			log.Printf("Invalid latitude value\n")
			return false
		}
		hr, err1 = strconv.Atoi(x[2][0:2])
		minf, err2 := strconv.ParseFloat(x[2][2:9], 32)
		if err1 != nil || err2 != nil {
			log.Printf("Unable to parse latitude info\n")
			return false
		}
		...
		// Longitude.
		if len(x[4]) < 10 {
			log.Printf("Invalid longitude info.\n")
			return false
		}
		hr, err1 = strconv.Atoi(x[4][0:3])
		minf, err2 = strconv.ParseFloat(x[4][3:10], 32)
		if err1 != nil || err2 != nil {
			log.Printf("Unable to parse longitude info\n")
			return false
		}
		...
		if err1 != nil {
			log.Printf("Invalid quality data\n")
			return false
		}
		...
		if err1 != nil {
			log.Printf("Invalid satellite data.\n")
			return false
		}
		...
		if err1 != nil {
			log.Printf("Invalid altitude data\n")
			return false
		}
...
func initRY835AI() {
	...
	log.Printf("Starting GPS stufff\n")
	go pollRY835AI()
}
