...
func GetCurrentAttitudeXYZ() (float32, float32, float32) {
	...
	eX := float32(math.Atan2(q0a*q1a+q2a*q3a, 0.5-q1a*q1a-q2a*q2a)) * 180 / math.Pi
	eY := float32(math.Asin(-2.0*(q1a*q3a-q0a*q2a))) * 180 / math.Pi
	eZ := float32(math.Atan2(q1a*q2a+q0a*q3a, 0.5-q2a*q2a-q3a*q3a)) * 180 / math.Pi

	attitudeX := math.Cos(eZ) * math.Cos(eY)
	attitudeY := math.Sin(eZ) * math.Cos(eY)
	attitudeZ := math.Sin(eY)
